...
export class AttachmentsService {
  ...
  async create(input: CreateInput) {
    const report = await this.prisma.microMixReport.findUnique({ where: { id: input.reportId } });
    if (!report) throw new NotFoundException('Report not found');

    const checksum = input.providedChecksum ?? (await this.sha256(input.file.path));
