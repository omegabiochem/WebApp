...
export class SystemAdminDashboardService {
  ...
  async listReports(user: any, query: AdminDashboardQuery) {
    ...
    // ✅ "ALL" really means every status in DashboardReport.
    // Only filter by status when the user explicitly selected one.
    if (status !== 'ALL') {
      where.status = status;
    }
  ...
  async rebuildDashboardReports(user: any) {
    ...
    return this.dashboardSync.rebuildAll();
  }
}
