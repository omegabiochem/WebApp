...
export type SterilityReportFormValues = {
  ...
  ftm_turbidity: string;
  ftm_observation: string;
  ftm_result: string;

  scdb_turbidity: string;
  scdb_observation: string;
  scdb_result: string;
...
export function useReportValidation(role?: Role, opts?: ValidationOpts) {
  ...
  const isEmpty = useCallback(
    (field: string, v: SterilityReportFormValues): boolean => {
      switch (field) {
        ...
        case "dateTested":
          return !v.dateTested;
        case "ftm_turbidity":
          return !v.ftm_turbidity?.trim();
        case "ftm_observation":
          return !v.ftm_observation?.trim();
        case "ftm_result":
          return !v.ftm_result?.trim();
        case "scdb_turbidity":
          return !v.scdb_turbidity?.trim();
        case "scdb_observation":
          return !v.scdb_observation?.trim();
        case "scdb_result":
          return !v.scdb_result?.trim();
  ...
  const validateAndSetErrors = useCallback(
    (values: SterilityReportFormValues): boolean => {
