...
function ClientsAdminTab() {
  ...
  const [details, setDetails] = useState<ClientDetailsSummary[]>([]);
  ...
  const filtered = useMemo(() => {
    ...
    return clients.filter((client) => {
      const detail = details.find((x) => x.clientCode === client.clientCode);
  ...
  async function load() {
    ...
    try {
      const [clientRows, detailRows] = await Promise.all([
        api<ClientRow[]>("/admin/clients"),
        api<ClientDetailsSummary[]>("/client-details"),
      ]);
  ...
  function openClient(clientCode: string) {
    navigate(`/manage-users/client/${encodeURIComponent(clientCode)}`);
  }
      ...
      <div className={cx(card, "overflow-hidden")}>
        ...
        >
          <div>
            ...
            <div className="text-xs text-slate-500 mt-1">
              Select a client to manage company information, schedule,
              reminders, billing, and contacts.
            </div>
          </div>
        ...
        {loading && clients.length === 0 ? (
          ...
          <>
            ...
            <div className="hidden lg:block">
              <div className="grid grid-cols-[1.1fr_2fr_1fr_1fr_1.3fr_1fr_0.8fr] gap-3 px-4 py-3 bg-slate-50 border-b border-slate-200 text-xs font-medium text-slate-500">
                <div>Client Code</div>
                <div>Client</div>
                <div>Users</div>
                <div>Active</div>
                ...
                {filtered.map((client) => {
                  ...
                  return (
                    <div
                      ...
                      className="grid grid-cols-[1.1fr_2fr_1fr_1fr_1.3fr_1fr_0.8fr] gap-3 items-center px-4 py-4 hover:bg-slate-50 transition"
                    >
                      <div>
                      ...
                      <div>
                        <span className="inline-flex rounded-full bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 px-2 py-1 text-xs">
                          {client.activeUsers}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-end">
                        <button
                          type="button"
                          className={btn.primary}
                          onClick={() => openClient(client.clientCode)}
                        >
                          Manage
                        </button>
                      </div>
