...
export default function COAReportFormView(props: COAReportFormViewProps) {
  ...
  const dashClass = (keyOrPrefix: string) =>
    hasOpenCorrectionKey(keyOrPrefix) ? "dash dash-red" : "";
                ...
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <div>
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <div
                        className={`flex-1 border-0 border-b border-black/60 min-h-[18px] font-medium ${
                          shouldBlurSignatures ? "blur-sm" : ""
                        }`}
                      >
                        {report?.testedBy?.toUpperCase() ?? ""}
                      </div>
                    </div>
