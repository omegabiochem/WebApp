...
// import { STATUS_TRANSITIONS } from "../../utils/microMixReportFormWorkflow";
...
// const DEFAULT_STATUS_LABELS: Partial<
//   Record<ReportStatus, { label: string; color: string }>
// > = {
...
//   CLIENT_NEEDS_PRELIMINARY_CORRECTION: { label: "Needs Correction", color: "bg-yellow-600" },
//   CLIENT_NEEDS_FINAL_CORRECTION: { label: "Needs Correction", color: "bg-yellow-600" },
...
//   UNDER_PRELIMINARY_RESUBMISSION_TESTING_REVIEW: { label: "Approve", color: "bg-blue-600" },

//   UNDER_FINAL_TESTING_REVIEW: { label: "Approve", color: "bg-green-600" },
//   UNDER_FINAL_RESUBMISSION_TESTING_REVIEW: { label: "Approve", color: "bg-blue-600" },

//   FINAL_TESTING_ON_HOLD: { label: "Hold", color: "bg-yellow-500" },
//   FINAL_TESTING_NEEDS_CORRECTION: { label: "Needs Correction", color: "bg-red-600" },
...
// type MicroReportFormProps = {
//   report: any;            // read-only view; parent passes the full report object
//   onClose: () => void;
//   onStatusChanged?: (newStatus: ReportStatus) => void; // optional callback to notify parent
// };
...
//   // local status so UI updates immediately after a change
//   const [localStatus, setLocalStatus] = useState<ReportStatus | string>(
//     (report?.status as ReportStatus) || "DRAFT"
//   );

//   useEffect(() => {
//     setLocalStatus((report?.status as ReportStatus) || "DRAFT");
//   }, [report?.status]);

//   // after printing, close this view
//   useEffect(() => {
//     const onAfterPrint = () => onClose?.();
//     window.addEventListener("afterprint", onAfterPrint);
//     return () => window.removeEventListener("afterprint", onAfterPrint);
...
//   // ---- status buttons logic (ONLY when canEdit is empty) ----
//   const currentStatus = (localStatus || "DRAFT") as ReportStatus;
//   const currentTransition = STATUS_TRANSITIONS[currentStatus];

//   const allowedNext: ReportStatus[] = useMemo(() => {
//     if (!currentTransition) return [];
//     // show only next statuses that the current role is allowed to set
//     return (currentTransition.next || []).filter((s) =>
//       currentTransition.canSet?.includes(role)
//     );
//   }, [currentTransition, role]);

//   const showStatusButtons =
//     !!currentTransition &&
//     (currentTransition.canEdit?.length ?? 0) === 0 &&
//     allowedNext.length > 0;

//   const [busy, setBusy] = useState<ReportStatus | null>(null);

//   async function handleActionClick(target: ReportStatus) {
//     try {
//       setBusy(target);
...
//       const url = `${API_BASE}/reports/micro-mix/${report.id}/status`;
...
//         },
//         body: JSON.stringify({ status: target, reason: "Status change from view page" }),
//       });
...
//       alert(`✅ Status changed to ${target}`);
//     } catch (err: any) {
...
//   function metaFor(target: ReportStatus) {
//     const meta = DEFAULT_STATUS_LABELS[target];
//     if (meta) return meta;
//     const pretty = target.replace(/_/g, " ").toLowerCase().replace(/^\w/, (m) => m.toUpperCase());
//     return { label: pretty, color: "bg-slate-600" };
//   }
...
//   const pathogenDefaults: PathRow[] = [
//     { checked: false, key: "E_COLI", label: "E.coli", result: "", spec: "Absent" },
//     { checked: false, key: "P_AER", label: "P.aeruginosa", result: "", spec: "Absent" },
//     { checked: false, key: "S_AUR", label: "S.aureus", result: "", spec: "Absent" },
//     { checked: false, key: "SALM", label: "Salmonella", result: "", spec: "Absent" },
//     { checked: false, key: "CLOSTRIDIA", label: "Clostridia species", grams: "3g", result: "", spec: "Absent" },
//     { checked: false, key: "C_ALB", label: "C.albicans", result: "", spec: "Absent" },
//     { checked: false, key: "B_CEP", label: "B.cepacia", result: "", spec: "Absent" },
//     { checked: false, key: "OTHER", label: "Other", grams: "", result: "", spec: "Absent" },
//   ];
...
//       {/* Controls (hidden on print) */}
//       <div className="no-print absolute top-2 right-2 flex flex-col items-end gap-2">
//         {/* Status actions (only when canEdit is empty) */}
//         {showStatusButtons && (
//           <div className="flex flex-wrap justify-end gap-2">
//             {allowedNext.map((target) => {
//               const { label, color } = metaFor(target);
//               const isBusy = busy === target;
//               return (
//                 <button
//                   key={target}
//                   onClick={() => handleActionClick(target)}
//                   className={`px-3 py-1 rounded text-white border ${color} ${
...
//       {/* Letterhead */}
//       <div className="mb-2 text-center">
//         <div className="font-bold tracking-wide text-[22px]" style={{ color: "blue" }}>
//           OMEGA BIOLOGICAL LABORATORY, INC.
//         </div>
//         <div className="text-[16px]" style={{ color: "blue" }}>
//           (FDA REG.)
...
//           <div className="px-2 border-r border-black flex items-center gap-1">
//             <div className="whitespace-nowrap font-medium">CLIENT:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.client || ""} readOnly disabled />
//           </div>
//           <div className="px-2 flex items-center gap-1">
//             <div className="whitespace-nowrap font-medium">DATE SENT:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={formatDateForInput(report?.dateSent) || ""} readOnly disabled />
//           </div>
//         </div>
...
//           <div className="px-2 border-r border-black flex items-center gap-1">
//             <div className="font-medium whitespace-nowrap">TYPE OF TEST:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.typeOfTest || ""} readOnly disabled />
//           </div>
//           <div className="px-2 border-r border-black flex items-center gap-1">
//             <div className="font-medium whitespace-nowrap">SAMPLE TYPE:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.sampleType || ""} readOnly disabled />
//           </div>
//           <div className="px-2 flex items-center gap-1">
//             <div className="font-medium whitespace-nowrap">FORMULA #:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.formulaNo || ""} readOnly disabled />
//           </div>
//         </div>
...
//         <div className="border-b border-black flex items-center gap-2 px-2 text-[12px] leading-snug">
//           <div className="w-28 font-medium">DESCRIPTION:</div>
//           <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.description || ""} readOnly disabled />
//         </div>
...
//           <div className="px-2 border-r border-black flex items-center gap-1">
//             <div className="font-medium whitespace-nowrap">LOT #:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.lotNo || ""} readOnly disabled />
//           </div>
//           <div className="px-2 flex items-center gap-1">
//             <div className="font-medium whitespace-nowrap">MANUFACTURE DATE:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={formatDateForInput(report?.manufactureDate) || ""} readOnly disabled />
//           </div>
//         </div>
...
//           <div className="px-2 border-r border-black flex items-center gap-1">
//             <div className="font-medium whitespace-nowrap">TEST SOP #:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.testSopNo || ""} readOnly disabled />
//           </div>
//           <div className="px-2 flex items-center gap-1">
//             <div className="font-medium whitespace-nowrap">DATE TESTED:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={formatDateForInput(report?.dateTested) || ""} readOnly disabled />
//           </div>
//         </div>
...
//           <div className="px-2 border-r border-black flex items-center gap-1">
//             <div className="font-medium">PRELIMINARY RESULTS:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={report?.preliminaryResults || ""} readOnly disabled />
//           </div>
//           <div className="px-2 flex items-center gap-1">
//             <div className="font-medium">PRELIMINARY RESULTS DATE:</div>
//             <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={formatDateForInput(report?.preliminaryResultsDate) || ""} readOnly disabled />
//           </div>
//         </div>
...
//         <div className="flex items-center gap-2 px-2 text-[12px] leading-snug">
//           <div className="font-medium whitespace-nowrap">DATE COMPLETED:</div>
//           <input className="flex-1 input-editable py-[2px] text-[12px] leading-snug" value={formatDateForInput(report?.dateCompleted) || ""} readOnly disabled />
//         </div>
//       </div>
...
//         {/* TBC */}
//         <div className="grid grid-cols-[27%_10%_17%_18%_28%] text-[12px] border-b border-black">
//           <div className="py-1 px-2 font-bold border-r border-black">Total Bacterial Count:</div>
//           <div className="py-1 px-2 border-r border-black"><div className="py-1 px-2 text-center"> x 10^0</div></div>
//           <div className="py-1 px-2 border-r border-black flex">
//             <input className="w-full input-editable px-1" value={report?.tbc_gram || ""} readOnly disabled />
//           </div>
//           <div className="py-1 px-2 border-r border-black flex">
//             <input className="w-1/2 input-editable px-1" value={report?.tbc_result || ""} readOnly disabled />
//             <div className="py-1 px-2 text-center">CFU/ml</div>
//           </div>
//           <div className="py-1 px-2 flex">
//             <input className="w-full input-editable px-1" value={report?.tbc_spec || ""} readOnly disabled />
//           </div>
//         </div>

//         {/* TMY */}
//         <div className="grid grid-cols-[27%_10%_17%_18%_28%] text-[12px]">
//           <div className="py-1 px-2 font-bold border-r border-black">Total Mold & Yeast Count:</div>
//           <div className="py-1 px-2 border-r border-black"><div className="py-1 px-2 text-center"> x 10^0</div></div>
//           <div className="py-1 px-2 border-r border-black flex">
//             <input className="w-full input-editable px-1" value={report?.tmy_gram || ""} readOnly disabled />
//           </div>
//           <div className="py-1 px-2 border-r border-black flex">
//             <input className="w-1/2 input-editable px-1" value={report?.tmy_result || ""} readOnly disabled />
//             <div className="py-1 px-2 text-center">CFU/ml</div>
//           </div>
//           <div className="py-1 px-2 flex">
//             <input className="w-full input-editable px-1" value={report?.tmy_spec || ""} readOnly disabled />
//           </div>
//         </div>
//       </div>

//       <div className="p-2 font-bold">PATHOGEN SCREENING (Please check the organism to be tested)</div>
...
//         </div>
//         {(report?.pathogens || pathogenDefaults).map((p: any) => (
//           <div key={p.key} className="grid grid-cols-[25%_55%_20%] border-b border-black text-[11px]">
//             <div className="py-[2px] px-2 border-r flex gap-2 items-center text-center">
//               <input type="checkbox" className="thick-box" checked={p.checked || false} readOnly disabled />
//               <span>{p.label}</span>
//             </div>
//             <div className="py-[2px] px-2 border-r flex gap-4 justify-center text-center">
//               <label>
//                 <input type="radio" className="thick-box" checked={p.result === "Absent"} readOnly disabled /> Absent
//               </label>
//               <label>
//                 <input type="radio" className="thick-box" checked={p.result === "Present"} readOnly disabled /> Present
//               </label>
//               <span className="ml-1">in 11g of sample</span>
//             </div>
//             <div className="py-[2px] px-2 text-center">{p.spec}</div>
...
//       {/* Legends / Comments */}
//       <div className="mt-2 text-[11px]">
//         <div className="font-bold border-black p-2" style={{ textDecoration: "underline" }}>
//           DENOTES: NA (Not Applicable) / N.G. (No Growth) / GM.(+)B Gram (+) Bacilli / GM.(+)C Gram (+) Cocci / GM.NEG Gram Negative / NT (Not
//           Tested) / TNTC (Too Numerous To Count)
//         </div>
//       </div>
...
//         <div className="p2 col-span-2 flex">
//           <div className="mb-1 font-medium">Comments:</div>
//           <input className="flex-1 border-0 border-b border-black/70 focus:border-blue-500 focus:ring-0 text-[12px] outline-none" value={report?.comments || ""} readOnly disabled />
//         </div>

//         <div className="p-2">
//           <div className="font-medium mb-2 flex items-center gap-2">
//             TESTED BY:
//             <input className="flex-1 border-0 border-b border-black/70 focus:border-blue-500 focus:ring-0 text-[12px] outline-none" value={report?.testedBy || ""} readOnly disabled />
//           </div>
//           <div className="font-medium mt-2 flex items-center gap-2">
//             DATE:
//             <input className="flex-1 border-0 border-b border-black/70 focus:border-blue-500 focus:ring-0 text-[12px] outline-none" value={formatDateForInput(report?.testedDate) || ""} readOnly disabled />
//           </div>
//         </div>
...
//           <div className="font-medium mb-2 flex items-center gap-2">
//             REVIEWED BY:
//             <input className="flex-1 border-0 border-b border-black/70 focus:border-blue-500 focus:ring-0 text-[12px] outline-none" value={report?.reviewedBy || ""} readOnly disabled />
//           </div>
//           <div className="font-medium mt-2 flex items-center gap-2">
//             DATE:
//             <input className="flex-1 border-0 border-b border-black/70 focus:border-blue-500 focus:ring-0 text-[12px] outline-none" value={formatDateForInput(report?.reviewedDate) || ""} readOnly disabled />
//           </div>
//         </div>
//       </div>
//     </div>
...
//   | "UNDER_PRELIMINARY_CORRECTION"
//   // | "PRELIMINARY_TESTING_REJECTED"
//   // | "PRELIMINARY_APPROVED"
//   | "UNDER_FINAL_TESTING_REVIEW"
//   | "FINAL_TESTING_ON_HOLD"
//   | "FINAL_TESTING_NEEDS_CORRECTION"
//   | "UNDER_FINAL_CORRECTION"
//   // | "FINAL_TESTING_REJECTED"
//   | "UNDER_QA_REVIEW"
//   | "QA_NEEDS_CORRECTION"
//   // | "QA_REJECTED"
//   | "UNDER_ADMIN_REVIEW"
...
//       "UNDER_CLIENT_PRELIMINARY_REVIEW",
//     ],
//     nextEditableBy: ["CLIENT", "MICRO", "ADMIN"],
//     canEdit: ["MICRO", "ADMIN"],
//   },
//   PRELIMINARY_TESTING_ON_HOLD: {
//     canSet: ["MICRO"],
...
//   UNDER_FINAL_TESTING_REVIEW: {
//     canSet: ["CLIENT"],
//     next: ["FINAL_TESTING_NEEDS_CORRECTION","FINAL_TESTING_NEEDS_CORRECTION","UNDER_QA_REVIEW"],
...
//     next: ["ADMIN_NEEDS_CORRECTION", "ADMIN_REJECTED","UNDER_CLIENT_FINAL_REVIEW"],
...
//             <div
//               id="f-typeOfTest"
//               className="px-2 border-r border-black flex items-center gap-1 relative"
//             >
//               <div className="font-medium whitespace-nowrap">TYPE OF TEST:</div>
//               {/* tiny floating badge; does not affect layout */}
...
//           <div
//             id="f-description"
//             className="border-b border-black flex items-center gap-2 px-2 text-[12px] leading-snug relative"
//           >
//             <div className="w-28 font-medium">DESCRIPTION:</div>
...
//           <div
//             id="f-dateCompleted"
//             className=" flex items-center gap-2 px-2 text-[12px] leading-snug relative"
//           >
//             <div className="font-medium whitespace-nowrap">DATE COMPLETED:</div>
...
//             <div
//               id="f-tbc_gram"
//               className="py-1 px-2 border-r border-black flex relative"
//             >
...
//                 {/* Third column (spec) */}
//                 <div
//                   className={`py-[2px] px-2 text-center ${
...
//             <div
//               id="f-testedDate"
//               className="font-medium mt-2 flex items-center gap-2 relative"
//             >
//               DATE:
