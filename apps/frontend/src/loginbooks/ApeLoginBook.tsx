// src/pages/Dashboards/ApeLoginBook.tsx
...
type Report = {
  ...
  // flattened APE details from /reports
  client?: string | null;
  ...
  // fallback keys if backend sends camelCase later
  reportNumberAssignedAt?: string | null;
  reportNumberAssignedBy?: string | null;
};
...
function getAssignedDate(r: Report) {
  return r.ReportnumberAssignedAt || r.reportNumberAssignedAt || null;
}

function getAssignedBy(r: Report) {
  return r.ReportnumberAssignedBy || r.reportNumberAssignedBy || null;
}
...
function exportCsv(rows: Report[]) {
  ...
  const lines = [
    ...
    ...rows.map((r) => {
      return [
        ...
        r.testSopNo ?? "",
        r.testReference ?? "",
        r.dateTested ? formatDate(r.dateTested) : "",
        r.dateCompleted ? formatDate(r.dateCompleted) : "",
        getAssignedDate(r) ? formatDate(getAssignedDate(r)) : "",
        getAssignedBy(r) ?? "",
        r.status ?? "",
        r.formNumber ?? "",
        r.formType ?? "",
  ...
  a.href = url;
  a.download = `ape-login-book-${new Date().toISOString().slice(0, 10)}.csv`;
...
function StatusPill({ value }: { value: string }) {
  ...
  if (v.includes("VOID"))
    return (
      <span className={`${base} bg-slate-100 text-slate-700 ring-slate-300`}>
        VOID
      </span>
    );
...
export default function ApeLoginBook() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // APE parent reports are loaded from /reports, same source as Micro Login Book.
        const all = await api<Report[]>("/reports");
        if (abort) return;

        const ape = all.filter((r) => r.formType === "APE");
        setRows(ape);
      } catch (e: any) {
        toast.error(e?.message || "Failed to load APE Login Book");
      } finally {
  ...
  const processed = useMemo(() => {
    ...
    const filtered = query
      ? withReportNo.filter((r) => {
          ...
          const code = (r.clientCode || "").toLowerCase();
          const description = (r.description || "").toLowerCase();
          const lot = (r.lotNo || "").toLowerCase();
          const formula = (r.formulaNo || "").toLowerCase();
          const sampleType = (r.sampleType || "").toLowerCase();
          const typeOfTest = (r.typeOfTest || "").toLowerCase();
          const sop = (r.testSopNo || "").toLowerCase();
          const reference = (r.testReference || "").toLowerCase();
          const status = (r.status || "").toLowerCase();
          const formNumber = (r.formNumber || "").toLowerCase();
          ...
          return (
            ...
            typeOfTest.includes(query) ||
            sop.includes(query) ||
            reference.includes(query) ||
            status.includes(query) ||
            formNumber.includes(query)
          );
        })
      : withReportNo;
      ...
      <div className="no-print mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            One-row-per-APE-report summary for quick lab log review.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <div className="relative w-full sm:w-[460px]">
            <input
              ...
              placeholder="Search report #, client, form #, sample, lot, formula, SOP, status…"
          ...
          <table className="w-full border-separate border-spacing-0 text-sm">
            <thead className="sticky top-0 z-10 bg-white">
              <tr className="text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                ...
                <th className="border-b px-4 py-3">Formula # / Sample type</th>
                <th className="border-b px-4 py-3">Type of test</th>
                <th className="border-b px-4 py-3">Status</th>
                <th className="border-b px-4 py-3">Assigned Date / Initials</th>
              </tr>
            </thead>
                ...
                displayRows.map((r, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="max-w-[340px] truncate text-slate-800">
                          {r.description || "-"}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="max-w-[300px] truncate text-slate-800">
                          {r.typeOfTest || "APE"}
                        </div>
                        <div className="text-[11px] text-slate-500">
                          {r.testSopNo || r.testReference || "-"}
                        </div>
                      </td>
